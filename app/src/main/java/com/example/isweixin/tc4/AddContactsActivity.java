...
public class AddContactsActivity extends Activity {
		...
		m_SaveBtn.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if ("".equals(m_EditName.getText().toString())) {
					Toast.makeText(AddContactsActivity.this, "请输入联系人姓名",
							Toast.LENGTH_SHORT).show();
				} else if ("".equals(m_EditNum.getText().toString())) {
					Toast.makeText(AddContactsActivity.this, "请输入联系人电话",
							Toast.LENGTH_SHORT).show();
				} else if (m_Type == 0) {
					// 新增联系人操作,放在线程中处理
	...
	class SaveContactTask extends AsyncTask<Void, Integer, Void> {
		...
		protected Void doInBackground(Void... params) {
			Utils.AddContact(m_EditName.getText().toString(), m_EditNum
					.getText().toString());
	...
	class ChangeContactTask extends AsyncTask<Void, Integer, Void> {
		...
		protected Void doInBackground(Void... params) {
			Utils.ChangeContact(m_EditName.getText().toString(), m_EditNum
					.getText().toString(), m_ContactId);
