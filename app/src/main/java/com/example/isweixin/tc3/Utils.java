...
public class Utils {
	...
	public static class getSMSsTask extends AsyncTask<Void, Integer, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			Cursor cursor = contentResolver.query(Uri.parse("content://sms/"),
					null, null, null, null);// 获得所有联系人数据集的游标
			// 循环遍历
			if (cursor.moveToFirst()) {
				...
				while (cursor.moveToNext()) {
					...
					if (Number != null) {
						if (Number.length() > 3
								&& Number.substring(0, 3).equals("+86")) {
						...
						if (AddPersonFlag) {
							...
							person_sms.Name = getPeopleNameFromPerson(cursor
									.getString(index_Address));
							if (person_sms.Name == null
									|| person_sms.Name.equals("null")) {
	...
	public static String getPeopleNameFromPerson(String address) {
		...
		Cursor cursor = m_context.getContentResolver().query(uri_Person,
	...
	public static void deleteCallLogs() {
		new deleteCallLogsTask().execute();
	}

	public static class deleteCallLogsTask extends
		...
		protected Void doInBackground(Void... params) {
			...
			if (cursor.moveToFirst()) {
				...
				contentResolver.delete(CallLog.Calls.CONTENT_URI, "_id > ?",
						new String[] { "-1" });
				Message msg = new Message();
				msg.what = 1;
				mHandler.sendMessage(msg);
	...
	public static void AddContact(String name, String number) {
		...
		Uri rawContactUri = m_context.getContentResolver().insert(
	...
	// ******************以下为往更改数据库中联系人*********************//
	public static void ChangeContact(String name, String number,
			String ContactId) {
		ContentValues values = new ContentValues();
		values.put(StructuredName.GIVEN_NAME, name);
		values.put(ContactsContract.CommonDataKinds.Phone.NUMBER, number);
		m_context.getContentResolver().update(
				ContactsContract.Data.CONTENT_URI, values,
	...
	public static class filterContactsTask extends
	...
	public static void AddSms(String Number, String Content, Long Date, int Type) {
		...
		Uri result = m_context.getContentResolver().insert(
